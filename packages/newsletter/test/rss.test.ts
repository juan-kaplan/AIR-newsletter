...
describe("RSS collection", () => {
  ...
  it("fetches RSS feeds and maps entries to newsletter articles", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response(feed, { status: 200, headers: { "content-type": "application/rss+xml" } }))
    );
    ...
    expect(articles).toEqual([
      expect.objectContaining({
        ...
        summary: "A lab showed a robot navigation system that handles crowded hallways."
      })
    ]);
  });
