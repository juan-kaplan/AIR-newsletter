...
const CLUB_CONTEXT = "undergraduate robotics club organizing competitions for Argentinian universities";

export function rankForRoboticsClub(articles: NewsletterArticle[], limit: number): ScoredArticle[] {
  return articles
    .map(scoreArticle)
...
export function scoreArticle(article: NewsletterArticle): ScoredArticle {
  ...
  score += addScore(text, ["open source", "ros", "simulation", "navigation", "perception", "manipulation", "autonomous", "drone", "rover"], 12, "buildable technical idea", reasons);
  score += addScore(text, ["research", "paper", "lab", "prototype", "dataset"], 8, "technical learning value", reasons);

  if (article.category === "competition" || article.category === "event") {
    score += 16;
    reasons.push(article.category === "competition" ? "competition source" : "event source");
  }
  ...
  score -= addPenalty(text, ["sponsored", "webinar", "funding round", "raises", "stock", "earnings"], 10);
  score -= addPenalty(text, ["surgical", "hospital", "warehouse", "logistics"], 4);
  score -= addPenalty(text, ["order worth", "deployment order", "million agreement", "hospitality food"], 12);

  return {
    ...article,
    score,
    selectionReason: reasons.slice(0, 3).join(", ") || `Relevant to ${CLUB_CONTEXT}`
  };
}

function addScore(text: string, keywords: string[], points: number, reason: string, reasons: string[]): number {
